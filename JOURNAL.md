...
- **[재검토] 수동 재동기화 실행 방식: `@Async`+상태 테이블 구현 보류, 동기
  (블로킹) 유지** — 전국 규모(3~5만 건) 대비로 `@Async`+상태 테이블 방향까지
  합의했었으나, 사용자가 재검토 후 1차 범위에서는 구현하지 않기로 뒤집음.
  이유: (a) 지금 Mock 데이터가 숙소 몇 개뿐이라 체감 효과가 없어 검증이 안
  되는 코드가 됨 (b) Supplier 어댑터·검색 API 등 아직 안 끝난 핵심 항목이
  우선순위상 더 급함. 결정만 `docs/architecture.md` "매핑 생성 트리거"에
  "대규모(3~5만 건) 시 async+상태테이블로 전환 필요 — 설계만 남기고 구현
  안 함"으로 기록하고 코드는 작성하지 않음.
  - 배치 upsert(JdbcTemplate 멀티로우, 위 항목)는 트리거 실행 방식과 무관한
    별개 최적화라 되돌리지 않고 그대로 유지 — "선택사항으로 남겨둔다"는
    사용자 판단.
...
- 배치 upsert 구현 직후, 사용자가 스스로 async+상태 테이블 구현을 재검토해
  "지금 단계에서는 과하다"고 판단 → Claude의 제안을 그대로 따르지 않고
  우선순위(Supplier 어댑터·검색 API 우선)를 사용자가 직접 재조정, 구현은
  보류하고 설계 판단만 문서화하도록 지시.
