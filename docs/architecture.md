...
완전한 헥사고날(`adapter/in`, `adapter/out` 풀 네이밍)은 이 프로젝트 규모(제한된
기간, 핵심 흐름 위주) 대비 과하다고 판단해, `supplier/`, `web/`처럼 익숙한 이름을
쓰되 포트-어댑터의
...
- 공급사가 다르면 같은 실제 숙소라도 내부 식별자를 공유하지 않는 것이 **기본
  동작**: 두 공급사 상품을 하나로 합치는 "중복 상품 병합"은 요구사항 문서에서
  선택 구현으로 명시했고, 병합 여부 판단(숙소명 유사도 등) 자체가 별도 설계
  주제라 1차 범위에서 제외.
...
발생할 수 있다 (요구사항 문서에서도 명시적으로 경고). 9090 등 별도 포트로 분리해
원인 추적을 단순하게 유지.

## WebClient: 왜 RestTemplate/RestClient가 아닌가

요구사항 문서에서 명시적으로 WebClient를 지정함 — 다수 공급사를 동시에
...
4. **외부 배치 프로세스로 분리** — 매핑 동기화 전용 별도 워커/크론잡.
   프로세스 관리, 배포 파이프라인, DB 상태 공유 등 인프라 복잡도가 이
   프로젝트 규모에 맞지 않음 — 단일 애플리케이션 내 `ApplicationRunner`로
   충분하다.
...
재동기화 트리거(엔드포인트) 구현까지 검토했었으나, 다시 보니 요구사항 문서는
"언제 호출할지 판단 + 근거"만 요구하고 있어 "앱 기동 시 1회"만으로도 충분한
답이 된다. 재동기화가 필요하면 앱 재시작이 사실상 같은 역할을 하고, 이
프로젝트 규모에서 별도 엔드포인트를 구현할 실익이 크지 않다고 판단해 제외했다.
...
1. 고객이 검색 결과를 기다릴 수 있는 체감 한계는 일반적인 웹 서비스 기준
   통상 수 초 이내다. 검색은 A/B를 병렬로 호출하므로 전체 응답 시간은
...
- A를 6초 지연으로 설정 → 4초에서 타임아웃 발생, `SupplierAvailabilityResult
  ...
  — "부분 실패 허용"(요구사항 문서에 명시된 동작)이 실제로 동작함을 확인.
...
- `partialFailures`는 애초에 "이 결과가 왜 불완전한지"를 호출하는 다음
  계층(프론트/BFF)에게 알려주는 용도로 설계된 필드다(요구사항 문서에 명시된
  동작). 검색 도중
...
근거 부족 때문):
- 요구사항 문서에 공급사별 rate limit 수치(요청/초 등)가
